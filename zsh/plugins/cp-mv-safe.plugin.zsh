...
cp_safe() {
  ...
  printf "\"%s\" " "$@[-1]" && \
  printf "\n" && \
  rsync -abXv -hhh --backup-dir=$HOME/tmp/rsync -e /dev/null --progress "$@[1,-2]" "$@[-1]"
}
...
mv_safe() {
  ...
  printf "\"%s\" " "$@[-1]" && \
  printf "\n" && \
  rsync -abXv -hhh --remove-source-files --backup-dir=$HOME/tmp/rsync -e /dev/null --progress "$@[1,-2]" "$@[-1]" && \
